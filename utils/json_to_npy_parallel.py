# create .npy file for each room of input json file as parallel
    ...
    for level in data["levels"]:
        for node in level["nodes"]:
            if node["type"] == "Object" and node["valid"] == 1:
                ...
                # TODO: a function with input (node, glob_bbox_min, )
                # TODO: return bbox_min, object_voxel,
                print ("start_time: ", str(datetime.datetime.now().time())[:-7])
                ...
                print ("end_time: ", str(datetime.datetime.now().time())[:-7])
                sys.exit(0)
